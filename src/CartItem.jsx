...
const CartItem = ({ onContinueShopping }) => {
  ...
  const calculateTotalAmount = () => {
    let total = 0
    cart.forEach((item) => {
      const cost = Number.parseFloat(item.cost.substring(1)) // Remove $ and convert to number
      ...
      <div>
        {cart.map((item) => (
          <div className="cart-item" key={item.name}>
            <img className="cart-item-image" src={item.image || "/placeholder.svg"} alt={item.name} />
